...
esp_err_t aplay_mp3(const char *path)
{
    ...
    ESP_LOGI(TAG_audio, "start to decode %s", path);
    ...
    if (sizeof(mp3_id3_header_v2_t) == fread(&tag, 1, sizeof(mp3_id3_header_v2_t), fp)) {
        if (memcmp("ID3", (const void *) &tag, sizeof(tag.header)) == 0) {
            //ESP_LOGI(TAG_audio, "start to Get ID3 head");
            int tag_len =
                ((tag.size[0] & 0x7F) << 21) +
                ((tag.size[1] & 0x7F) << 14) +
                ((tag.size[2] & 0x7F) << 7) +
    ...
    //ESP_LOGI(TAG_audio, "start to Start MP3 decoding");
    /* Start MP3 decoding */
    int bytes_left = 0;
    int outOfData = 0;
    ...
    do {
        ...
        /* MP3decode */
        MP3Decode(mp3_decoder, &read_ptr, &bytes_left, (int16_t *) output, 0);
        //int mp3_dec_err = MP3Decode(mp3_decoder, &read_ptr, &bytes_left, (int16_t *) output, 0);
        // if( mp3_dec_err == 0){
        //     ESP_LOGI(TAG_audio, "MP3Decode OK");
        // }

        /* Get MP3 frame info and configure I2S clock */
        //ESP_LOGI(TAG_audio, "start to Get MP3 frame info");
        ...
        if(frame_info.nChans ==  1) {
            //ESP_LOGI(TAG_audio, "start to convert mono to stereo");
        ...
        if (sample_rate != frame_info.samprate) {
            ...
            //ESP_LOGI(TAG_audio, "Configure I2S clock again.sample_rate = %d,bits_cfg=%d,channel=%d",sample_rate,bits_cfg,channel);
        }
        ...
        //esp_err_t i2s_ret = i2s_write(I2S_NUM_0, output, output_size, &i2s_bytes_written, portMAX_DELAY);
        // if(i2s_ret == ESP_OK){
        //     ESP_LOGI(TAG_audio, "Write decoded data to audio decoder OK!output_size= %d",i2s_bytes_written);
        // }
    } while (!outOfData);
